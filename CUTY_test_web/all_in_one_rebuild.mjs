...
// 1. Extract the old example jobs from the current employer fields
const extractRegex = /"(\w{3,4})"\s*:\s*\{[\s\S]*?employer:\s*"(.*?)"/g;
const oldEmployers = {};
let match;
while ((match = extractRegex.exec(content)) !== null) {
    oldEmployers[match[1]] = match[2];
}
console.log(`Found ${Object.keys(oldEmployers).length} old employers to save as exampleJobs.`);

// 2. Perform the official legal text replacements
...
content = content.replace(/("441":\s*\{[\s\S]*?salary:\s*").*?(",)/, `$1당해연도 법정 최저임금 적용 (GNI 요건 미적용)$2`);
content = content.replace(/("261":\s*\{[\s\S]*?employer:\s*").*?(",)/, `$1로펌 및 법률 사무소 등. (국민고용보호 심사제도 비적용 대상)$2`);

// 3. Inject the saved oldEmployers as exampleJobs
let injectedCount = 0;
...
console.log(`Rebuild and Restoration complete! Injected ${injectedCount} exampleJobs fields.`);
